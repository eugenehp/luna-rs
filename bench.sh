...
detect_cpu() {
    ...
    case "$OS" in
        Darwin)
            result="$(sysctl -n machdep.cpu.brand_string 2>/dev/null || true)"
            [ -z "$result" ] && result="$(sysctl -n hw.model 2>/dev/null || true)"
            ;;
        Linux)
            result="$(grep -m1 'model name' /proc/cpuinfo 2>/dev/null | sed 's/model name\s*:\s*//' || true)"
            [ -z "$result" ] && result="$(lscpu 2>/dev/null | grep -m1 'Model name' | sed 's/.*Model name:\s*//' || true)"
            [ -z "$result" ] && result="${ARCH} $(nproc 2>/dev/null || echo '?')cores"
            ;;
    esac
        ...
        Darwin)
            ...
            if [[ "$chip" == *"Apple"* ]]; then
                ...
                system_profiler SPDisplaysDataType 2>/dev/null \
                    | grep -m1 'Chipset Model' | sed 's/.*Chipset Model: //' \
                    || echo "Unknown"
            fi
            ;;
        Linux)
            if command -v nvidia-smi &>/dev/null; then
                nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1 || echo "Unknown NVIDIA"
...
# ─── Run a benchmark, capturing stdout→json, stderr→log ─────────────────────
# Returns 0 if JSON output is valid, 1 otherwise.
run_bench() {
    local label="$1"
    local json_file="$2"
    local log_file="$3"
    shift 3
    # remaining args are the cargo command

    echo "━━━ Running $label benchmark ━━━"

    # Run directly — no timeout wrapper needed (cargo itself will finish or crash)
    if "$@" > "$json_file" 2>"$log_file"; then
        # Check the JSON is actually valid
        if python3 -c "import json,sys; json.load(open(sys.argv[1]))" "$json_file" 2>/dev/null; then
            echo "  ✓ $label results → $json_file"
            return 0
        else
            echo "  ⚠  $label benchmark produced invalid JSON output"
            echo "  stdout:"
            head -20 "$json_file"
            echo "  stderr:"
            head -20 "$log_file"
        ...
        echo "  ⚠  $label benchmark exited with error (see $log_file)"
...
if run_bench "CPU" "$CPU_JSON" "$CPU_LOG" \
    cargo run --release --example benchmark --features "$CPU_FEATURES" \
    -- --variants "$VARIANTS" --warmup "$WARMUP" --runs "$RUNS" --json; then
    CPU_OK=true
fi
echo ""
...
if $GPU_AVAILABLE; then
    if run_bench "GPU" "$GPU_JSON" "$GPU_LOG" \
...
python3 - "$RUN_DIR" "$GPU_OK" "$CPU_NAME" "$GPU_NAME" "$DATE_HUMAN" <<'PYEOF'
import json, sys
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
for i, variant in enumerate(variants):
    ...
    ax.plot(chans, cpu_ms, "o-", color=CPU_COLOR, linewidth=2, markersize=6, label="CPU", zorder=3)
    if gpu_data:
        gpu_cs = gpu_data[i]["channel_scaling"]
        gpu_ms = [c["mean_ms"] for c in gpu_cs]
        ax.plot(chans, gpu_ms, "s-", color=GPU_COLOR, linewidth=2, markersize=6, label="GPU", zorder=3)
    ax.set_xlabel("Channels"); ax.set_ylabel("Latency (ms)")
    ax.set_title(f"LUNA-{variant.capitalize()}", fontsize=12, fontweight="bold")
    ax.legend(framealpha=0.8); ax.grid(True, alpha=0.3); ax.set_axisbelow(True); ax.set_xticks(chans)
...
bars = ax.bar(x - width/2, cpu_load, width, label="CPU", color=CPU_COLOR, alpha=0.85, edgecolor="white", linewidth=0.5)
if gpu_data:
    gpu_load = [d["load_ms"] for d in gpu_data]
    bars_g = ax.bar(x + width/2, gpu_load, width, label="GPU", color=GPU_COLOR, alpha=0.85, edgecolor="white", linewidth=0.5)
for bar in bars:
    h = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2, h + 5, f"{h:.0f}", ha="center", va="bottom", fontsize=9, fontweight="bold")
if gpu_data:
    for bar in bars_g:
        h = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2, h + 5, f"{h:.0f}", ha="center", va="bottom", fontsize=9, fontweight="bold")
ax.set_xlabel("LUNA Variant"); ax.set_ylabel("Load Time (ms)")
ax.set_title("Model Load Time — CPU vs GPU", fontsize=13, fontweight="bold")
ax.set_xticks(x); ax.set_xticklabels([f"LUNA-{v.capitalize()}" for v in variants])
ax.legend(framealpha=0.8); ax.grid(axis="y", alpha=0.3); ax.set_axisbelow(True)
fig.text(0.99, 0.01, date_str, ha="right", fontsize=8, alpha=0.5)
plt.tight_layout()
...
fig, ax = plt.subplots(figsize=(10, 6))
positions = []; labels = []; all_data = []; colors = []; pos = 1
for i, variant in enumerate(variants):
    all_data.append(cpu_data[i]["inference"]["all_ms"])
    positions.append(pos); labels.append(f"{variant.capitalize()}\nCPU"); colors.append(CPU_COLOR); pos += 1
    if gpu_data:
        all_data.append(gpu_data[i]["inference"]["all_ms"])
        positions.append(pos); labels.append(f"{variant.capitalize()}\nGPU"); colors.append(GPU_COLOR); pos += 1
    pos += 0.5
...
print(f"  ╠{'─'*60}╣")
for d in cpu_data:
    v = d["variant"].capitalize(); inf = d["inference"]
